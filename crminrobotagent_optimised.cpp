...
#define NUMBER_FEATURES_IN_FEATUREVECTOR 6 // number of features in feature vector
...
CRMinRobotAgentOptimised::CRMinRobotAgentOptimised(unsigned robotId, unsigned numfeatures)
{
    ...
    assert(NUMBER_FEATURES_IN_FEATUREVECTOR == numfeatures);
            ...
            /*if (m_uHistoryTcells > 0)
            {
                ...
                for(pop_index = listlistTcells.begin(); pop_index != listlistTcells.end(); ++pop_index)
                {
                        ...
                        it_tcells1 != (*pop_index).end(); ++it_tcells1)
                    {
                        ...
                        if ((double)hammingdistance / (double) NUMBER_FEATURES_IN_FEATUREVECTOR < 2.0/6.0)
...
void CRMinRobotAgentOptimised::UpdateTcellList(unsigned hammingdistance)
{
    ...
    if(m_fInternalRobotTimer == (MODELSTARTTIME+1))
    {
        ...
        if(hammingdistance == NUMBER_FEATURES_IN_FEATUREVECTOR)
        {
...
double CRMinRobotAgentOptimised::NegExpDistAffinity(unsigned int v1, unsigned int v2, double k)
{
    ...
    /*if((((double)hammingdistance) / ((double) NUMBER_FEATURES_IN_FEATUREVECTOR)) < (2.0f/6.0f))
        return exp(-(1.0f/k) * (((double)hammingdistance) / ((double) NUMBER_FEATURES_IN_FEATUREVECTOR)));
